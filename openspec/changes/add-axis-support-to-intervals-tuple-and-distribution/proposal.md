...
- Add an internal, non-public `is_valid_intervals_chain(..., axis=None)` hook used by `intervals_tuple`; its provisional one-dimensional validity check always returns `True` so stronger validation can be introduced later without changing the tuple API.
...
- Core and partial implementation: tuple and distribution logic shares axis dispatch and masked result packing while retaining direct one-dimensional kernels and partial gap-aware tuple calculations.
- Internal API: a non-exported interval-chain validation helper is introduced and called before tuple transformation.
