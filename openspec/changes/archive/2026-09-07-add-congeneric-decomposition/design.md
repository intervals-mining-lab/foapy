...
- Chain downstream stages through their outputs: `intervals_chains(CS, binding, chain_mode)` takes the decomposition, `intervals_tuples(chains, binding, tuple_mode)` takes the result of `intervals_chains()`, and `intervals_distributions(tuples)` takes the result of `intervals_tuples()`. `alphabet` and `order` also take `CS` directly (see D2.1 and D2.2).
...
### D2.2: All interval stages take upstream output directly, not `S`
...
3. **`intervals_distributions(tuples)`** (was `intervals_distributions(CS, binding, chain_mode, tuple_mode)`): the binding, chain mode, and tuple mode are already encoded in `intervals_tuples()`'s values. The final stage therefore takes that tuple matrix directly and excludes its structural `0` padding before counting interval frequencies. No mode parameter or earlier-stage input is needed.

**Result (final)**: only `sequences(S)` takes the raw source sequence. Every other `congenerics.*` function takes upstream output directly — `alphabet`/`order`/`intervals_chains` take `CS`, `intervals_tuples` takes `intervals_chains`'s output, and `intervals_distributions` takes `intervals_tuples`'s output. A caller building the full pipeline computes `CS = sequences(S)` once and threads each stage's output into the next — no stage recomputes an earlier stage's work.
...
- `intervals_distributions(tuples)`: takes the padded tuple matrix directly (see D2.2), excludes structural `0` padding, then uses a single vectorized scatter-add (`numpy.add.at` over every row's real tuple entries at once) to compute all `m` row-histograms simultaneously, instead of one `foapy.core.intervals_distribution` call per row.
...
- **[Risk, resolved by D2.2]** Recomputing `sequences(S)` or any intermediate interval stage inside later `congenerics.*` calls added avoidable work under D2's original "independent `S` input" contract. → **Resolution**: D2.2 changed every interval stage to take its upstream result directly, eliminating internal recomputation throughout the full pipeline.
