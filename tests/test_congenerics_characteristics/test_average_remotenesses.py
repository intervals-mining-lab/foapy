...
class TestAverageRemotenesses(MACharacteristicsTest):
    """
    Test list for average_remotenesses calculate
    """

    epsilon = np.float_power(10, -15)

    def target(self, X, dtype=None):
        return average_remotenesses(X)
    ...
    def AssertInEquality(self, X):
        ...
        for b in [binding_constant.start, binding_constant.end]:
            for m in modes:
                ...
                g = average_remotenesses(intervals_seq)
                H = identifying_informations(intervals_seq)
