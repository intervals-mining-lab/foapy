...
class TestGeometricMeans(MACharacteristicsTest):
    """
    Test list for geometric_means calculate
    """

    epsilon = np.float_power(10, -15)

    def target(self, X, dtype=None):
        return geometric_means(X)
    ...
    def AssertInEquality(self, X):
        ...
        for b in [binding_constant.start, binding_constant.end]:
            for m in modes:
                ...
                delta_g = geometric_means(intervals_seq)
                delta_a = arithmetic_means(intervals_seq)
                err_msg = (
